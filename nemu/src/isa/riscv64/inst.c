...
static vaddr_t *get_csr(uint32_t imm)
{
  switch(imm)
  {
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, uint32_t shamt = BITS(imm,4,0)&0x1f;R(rd) = (int32_t)src1 >> shamt);//这个好像有问题
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, s->dnpc = isa_raise_intr(0xb, s->pc);IFDEF(CONFIG_ETRACE,printf("ecall:entrance=0x%x\n",s->dnpc));cpu.csr.mstatus = 0x1800;);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, word_t temp = CSR(imm);CSR(imm) = CSR(imm)|src1;R(rd) = temp);
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, s->dnpc = cpu.csr.mepc;IFDEF(CONFIG_ETRACE,printf("mret:exit=0x%x\n",s->dnpc)));
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(rd) = src1|imm);
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1+imm,1),8));
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, R(rd) = CSR(imm);uint32_t i = s->isa.inst;uint32_t rs1_num = BITS(i, 19, 15);CSR(imm) = CSR(imm)|((rs1_num<<27)>>27));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (((uint64_t)(uint32_t)src1 * (uint64_t)(uint32_t)src2) >> 32));
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, word_t temp = CSR(imm);CSR(imm) = CSR(imm)&~src1;R(rd) = temp);
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, R(rd) = CSR(imm);uint32_t i = s->isa.inst;uint32_t rs1_num = BITS(i, 19, 15);CSR(imm) = CSR(imm)&~((rs1_num<<27)>>27));
